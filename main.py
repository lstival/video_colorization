...
video_name = "C:/video_colorization/data/videos/gray/video_cows.mp4"
...
example_path = "C:/video_colorization/data/train/DAVIS_val/cows/"

# example_img = Image.open(f"{example_path}{str(count-1).zfill(5)}.jpg", )
example_img = Image.open("C:/video_colorization/data/train/DAVIS_val/cows/00100.jpg")
example_img = transforms.functional.pil_to_tensor(example_img.resize(image_size)).to(device)
example_img = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])(example_img.type(torch.float32))

# ================ Read Model =====================
# predict the color for each frame in the video

from simple_color_model import *
...
frame_2_video(colored_frames_save, f"{colored_video_path}/cow_colored.mp4")
